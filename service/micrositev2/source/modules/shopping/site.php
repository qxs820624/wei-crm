...
class ShoppingModuleSite extends WeModuleSite {
	...
	public function doWebGoods() {
		...
		if ($operation == 'post') {
			...
			**/
			...
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('shopping_goods') . " WHERE weid = '{$_W['weid']}' $condition");
	...
	public function doWebOrder() {
		...
		if ($operation == 'display') {
			...
			if ($status != '-1') {
				$condition .= " AND status = '".intval($status)."' AND sendtype = 1";
			}
	...
	public function doMobileConfirm() {
		...
		if (!empty($cart['goods'])) {
			$goods = pdo_fetchall("SELECT id, title, thumb, marketprice, unit, total, sales FROM ".tablename('shopping_goods')." WHERE id IN ('".implode("','", array_keys($cart['goods']))."')");
		}
		if (empty($goods)) {
			message('抱歉，您的购物车里没有任何商品，请先购买！', $this->createMobileUrl('list'), 'error');
		}
