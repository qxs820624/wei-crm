...
class SystemAction extends BackAction{
  ...
  PRIMARY KEY (`id`)
	...
	public function index(){
		...
		$where['status']=1;
		$order['sort']='asc';
		$navs=M('Node')->where($where)->order($order)->select();
		...
		$wheres['display']=2;
		$wheres['status']=1;
		$wheres['pid']=array(in,$pid);
		$wheres['level']=2;
		$navx=M('Node')->where($wheres)->order($order)->select();
		...
		$role=M('Role')->where(array('id'=>$roleid))->getField('name');
	...
	public function main(){
        ...
        $updatehost = 'http://api.idz.pw/update.php';
        $updatehosturl = $updatehost . '?a=client_check_time&v=' . $v . '&u=' . $hosturl;
         $vertime['time'] =date("Y-m-d", file_get_contents($updatehosturl));
        if($domain_time == '0'){
            $vertime = '[授权版本：授权已过期，请联系客服QQ:263122634]';
        }else{
            $vertime = '授权版本：(高级商业版)-更新服务截止：(' .$vertime['time'] . ')';
        }
		$this->assign('vertime', $vertime); 
        $updatehost = 'http://api.idz.pw/update.php';
        $updatehosturl = $updatehost . '?a=chanage&v=' . $v . '&u=' . $hosturl;
		$dnr= file_get_contents($updatehosturl);
		$this->assign('dnr', $dnr); 	
		$version = CONF_PATH . 'version.php';
        $ver = include($version);
        $ver = $ver['ver'];
        ...
        if($lastver !== $ver){
            $updateinfo = ('<p class="red">官方最新版本为：花粉微信营销系统v ' . $lastver) . '</p><span>
		   <a href="javascript:if(confirm(\'升级前,请确认已经做好数据库和程序备份!\'))location=\'./index.php?g=System&m=Update&a=updatenow\'">点击这里在线升级</a>
            ...
            $updateinfo = ('<p class="red">官方最新版本为：花粉微信系统v ' . $lastver) . '</p><span>已经是最新系统 不需要升级</span>';
        }
	...
	public function doSqlUpdate(){
		...
		if (intval($rt['success'])<1){
			...
			if (empty($status) && empty($_SESSION['sql_log_flag'])) {
			...
			//record to database
			if ($rt['time']){
				if (empty($_SESSION['sql_log_flag'])) {
	...
	public function getcontents($url, $recu = 0) {
