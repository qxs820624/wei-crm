...
// 					$userinfo_model->where(array('id' => $thisUser['id']))->save($userRow);
			...
			$model->sendTempMsg('TM00184', array('href' => U('Store/my',array('token' => $this->token, 'success' => 1, 'twid' => $this->_twid), true, false, true), 'first' => '购买商品提醒', 'ordertape' => date("Y年m月d日H时i分s秒"), 'ordeID' => $orderid, 'remark' => '下单成功，请尽快支付订单！'));
			if ($totalprice) {
