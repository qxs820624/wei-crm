...
if ($operation == 'display') {
    ...
    $sql = "select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,s.storename,s.address as storeaddress from " . tablename('ewei_shop_creditshop_log') . " log " . " left join " . tablename('ewei_shop_member') . " m on m.openid = log.openid and m.uniacid=log.uniacid" . " left join " . tablename('ewei_shop_member_address') . " a on a.id = log.addressid" . " left join " . tablename('ewei_shop_store') . " s on s.id = log.storeid" . " left join " . tablename('ewei_shop_creditshop_goods') . " g on g.id = log.goodsid" . " where 1 {$condition} ORDER BY log.createtime desc ";
    ...
    foreach ($list as &$row) {
        ...
        if (!empty($row['addressid'])) {
            ...
            $row['address'] = array(
                ...
                'carrier_storename' => $row['storename'],
    ...
    if ($_GPC['export'] == 1) {
        ...
        $columns = array(
            ...
            array(
                'title' => '邮寄地址',
                'field' => 'addressinfo_province',
                'width' => 12
            ),
            array(
                'title' => '',
                'field' => 'addressinfo_city',
                'width' => 12
            ),
            array(
                'title' => '',
                'field' => 'addressinfo_area',
                'width' => 12
            ),
            array(
                'title' => '',
                'field' => 'addressinfo_address',
                'width' => 24
            ),
            array(
                'title' => '',
                'field' => 'addressinfo_realname',
                'width' => 12
            ),
            array(
                'title' => '',
                'field' => 'addressinfo_mobile',
                'width' => 12
            ),
            array(
                'title' => '兑换门店',
                'field' => 'storeinfo_storename',
                'width' => 12
            ),
            array(
                'title' => '',
