...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_commission_level', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
        ...
        message('更新等级成功！', $this->createPluginWebUrl('commission/level', array(
            'op' => 'display'
        )), 'success');
    }
    ...
    if (empty($level)) {
        message('抱歉，等级不存在或是已经被删除！', $this->createPluginWebUrl('commission/level', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ewei_shop_commission_level', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    plog('commission.level.delete', "删除分销商等级 ID: {$id} 等级名称: {$level['levelname']}");
    message('等级删除成功！', $this->createPluginWebUrl('commission/level', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
