...
//require '../addons/ewei_money/jssdk.php';
class Ewei_moneyModuleSite extends WeModuleSite {
  ...
  public function doWebSysset() {
    ...
    public function doMobileIndex() {
		...
		$signPackage = $this->GetSignPackage($appIdShare,$appSecretShare);
        //print_r('<pre>');print_r($signPackage);
		$sql="SELECT * FROM " . tablename($this->tablefans) . " WHERE rid = '{$rid}' and from_user = '{$openid}' ";
        $ifans = pdo_fetch($sql);
		//print_r($sql);print_r($ifans);
    ...
    private function getJsApiTicket($appId,$appSecret) {
        ...
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($ticket) {
                ...
                cache_write("ewei.money.api_share.json::".$_W['uniacid'], iserializer(array("appid"=>$appId,"appsecret"=>$appSecret)));
            }
        } else {
