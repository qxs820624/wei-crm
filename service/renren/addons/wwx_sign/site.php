...
define("SIGN_MODULENAME", "wwx_sign");
...
class Wwx_SignModuleSite extends WeModuleSite
{
    public $uniacid;
    public $oauth;
    ...
    function __construct()
    {
        ...
        $wechat = pdo_fetch("select * from ".tablename('account_wechats')." where uniacid=:uniacid limit 1",array(":uniacid"=>$this->uniacid));
        if($wechat){
            $appid = $wechat['key'];
...
*/
    public function  doWebSign()
    {
        ...
        if ($operation == 'display') {
            ...
            $psize = 20;
            $list = pdo_fetchall("SELECT * FROM " . tablename(CRUD::$table_sign) . " WHERE uniacid =:uniacid  ORDER BY createtime DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':uniacid' => $this->uniacid));
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename(CRUD::$table_sign) . " WHERE uniacid =:uniacid ", array(':uniacid' => $this->uniacid));
...
*/
    public function  doWebSystemaward()
    {
        ...
        if (checksubmit()) {
            ...
            message('更新用户积分成功！', $this->createWebUrl('SignUser', array(
                'sid' => $sid
            )), 'success');
        }
...
*/
    public function doWebAnalyse(){
        global $_GPC;
        $signs=pdo_fetchall("select * from ".tablename(CRUD::$table_sign)." where uniacid=:uniacid",array(':uniacid'=>$this->uniacid));
        $sid=$_GPC['sid'];
		$do=$_GPC['do'];
        if(empty($sid)){
            $sid=$signs[0]['id'];
...
*/
    public function  doMobileIndex()
    {
		...
		$myheadimg = $this->synmembercredit($userInfo['openid']);
		$signcredit = $myheadimg['credit1'];
