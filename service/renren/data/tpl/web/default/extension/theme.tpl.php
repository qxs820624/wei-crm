					...
					for (var branch in theme_branches) {
						...
						if (theme_branches[branch].id != current_branch) {
							if ( parseInt(theme_branches[branch].displayorder) > current_branch_sort) {
								branch_info += '<a href="' + trade_link + '&branch=' + theme_branches[branch].id + '" class="btn btn-warning js-module-trade" data-price="' + theme_branches[branch].upgrade_price + '">付费升级到分支最新版</a></div></div>';
							} else {
							...
							if ( parseFloat(theme_branches[branch].version.version) > parseFloat(current_version) ) {
								branch_info += '<a href="' + trade_link + '&branch=' + theme_branches[branch].id + '" class="btn btn-success js-module-trade" data-price="0">免费升级到分支最新版</a></div></div>';
							} else {
								branch_info += '<a href="javascript:;" class="btn btn-default">已是最新版，无需升级</a></div></div>';
							}
						}
