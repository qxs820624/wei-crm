...
//商品展示
if($status==1){
	$rule = pdo_fetch("select id from " . tablename('rule') . 'where uniacid=:uniacid and module=:module and name=:name', array(':uniacid' => $_W['uniacid'], ':module' => 'feng_fightgroups', ':name' => "拼团核销入口"));
	if ($rule) {
	...
	if (checksubmit('keysubmit')) {
		...
		if (empty($rule)) {
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => '拼团核销入口', 'module' => 'feng_fightgroups', 'displayorder' => 0, 'status' => 1);
			pdo_insert('rule', $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'feng_fightgroups', 'content' => trim($keyword), 'type' => 1, 'displayorder' => 0, 'status' => 1);
	...
	if ($op == 'display') {
		...
		if(checksubmit('storesubmit')){
			...
			if($id){
				pdo_update('tg_store',$data,array('id'=>$id));
			}else{
		...
		$id = $_GPC['id'];
		pdo_delete('tg_store',array('id'=>$id));
	...
	if ($op == 'display') {
		...
		$id = $_GPC['id'];
		pdo_delete('tg_saler',array('id'=>$id));
		message('删除成功！', referer(), 'success');
	}
	include $this -> template('web/hexiao');
}elseif($status==4){
