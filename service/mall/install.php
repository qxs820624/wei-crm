<?php
/**
 * [WeEngine MALL System] Copyright (c) 2014 WE7.CC
...
if($action == 'env') {
	...
	$ret['php']['short_open_tag']['ok'] = ini_get('short_open_tag');
	if(empty($ret['php']['short_open_tag']['ok']) || strtolower($ret['php']['short_open_tag']['ok']) == 'off') {
		$ret['php']['short_open_tag']['value'] = '<span class="glyphicon glyphicon-ok text-success"></span>';
		$ret['php']['short_open_tag']['class'] = 'success';
	} else {
		$ret['php']['short_open_tag']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
		$ret['php']['short_open_tag']['class'] = 'danger';
		$ret['php']['short_open_tag']['remark'] = '请禁用可以使用 php 代码标志的缩写形式，配置php.ini中short_open_tag = Off';
	}
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if(empty($ins) || !is_array($ins)) {
					die('<script type="text/javascript">alert("连接不到云服务器, 请稍后重试！");history.back();</script>');
				}
				if($ins['error']) {
					die('<script type="text/javascript">alert("链接云服务失败, 错误为: ' . $ins['error'] . '！");history.back();</script>');
				}
				$archive = $ins['files'];
				if(!$archive) {
...
VER;
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/web') && file_exists($verfile) && file_exists($dbfile)) {
				...
				if ($dat['permission'] && is_array($dat['permission'])) {
					foreach ($dat['permission'] as $menu_sql) {
						$menu_sql = str_replace('`ims_', '`'.$db['prefix'], $menu_sql);
						mysql_query($menu_sql);
					}
				}
			...
			$salt = local_salt(8);
			$password = md5("{$user['password']}-{$salt}-{$authkey}");
			mysql_query("INSERT INTO {$db['prefix']}user (username, password, salt, joindate, role_id, status) VALUES('{$user['username']}', '{$password}', '{$salt}', '" . time() . "', 1, 2)");
			local_mkdirs(IA_ROOT . '/data');
...
if($action == 'finish') {
	...
	cache_build_setting();
	cache_build_rolez_menus();
	cache_clean_address_tree_in_use();
	cache_clean_address_tree_has_store();
	cache_clean();
	tpl_install_finish();
}
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 微擎Mall - 开源的B2C、O2O商城系统</title>
		...
		<div class="container">
			<div class="header" style="margin:15px auto;">
				<ul class="nav nav-pills pull-right" role="tablist">
					<li role="presentation" class="active"><a href="javascript:;">安装微擎Mall系统</a></li>
			...
			<div class="footer" style="margin:15px auto;">
				...
				<div class="text-center">
					Powered by <a href="http://www.we7.cc"><b>微擎</b></a> v1.0 &copy; 2015 <a href="http://www.we7.cc">www.we7.cc</a>
				</div>
			</div>
		</div>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				...
				<p>
					感谢您选择微擎Mall - 开源的B2C、O2O商城系统（以下简称WE7，WE7基于 PHP + MySQL的技术开发，全部源码开放。 <br />
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="we7_mall">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-4">
						<div class="checkbox">
							<label class="text-success">
								<input type="checkbox" name="db[demo]" value="true" checked="checked">
								安装体验数据
							</label>
						</div>
					</div>
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“微擎Mall - 开源的B2C、O2O商城系统”，您现在可以: <a target="_blank" class="btn btn-success" href="./web/index.php">进入商城后台</a>
	</div>
EOF;
	tpl_frame();
}
