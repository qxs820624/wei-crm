...
define('shopwwi_ROOT', substr(dirname(__FILE__), 0, -3));
$data = require(shopwwi_ROOT.'/data/cache/setting.php');
require_once shopwwi_ROOT.'/data/config/config.ini.php';
...
if(!defined('IN_UC')) {
	...
	$action = $get['action'];
	require_once shopwwi_ROOT.'/uc_client/lib/xml.class.php';
...
class uc_note {
	...
	function _serialize($arr, $htmlon = 0) {
		if(!function_exists('xml_serialize')) {
			include_once shopwwi_ROOT.'/uc_client/lib/xml.class.php';
		}
		return xml_serialize($arr, $htmlon);
	}

	function uc_note() {
		$this->appdir = shopwwi_ROOT;
		$this->db = $GLOBALS['db'];
	...
	function updatecredit($get, $post) {
		...
		require_once shopwwi_ROOT.'/uc_client/data/cache/creditsettings.php';
	...
	function updatecreditsettings($get, $post) {
		...
		$cachefile = shopwwi_ROOT.'/uc_client/data/cache/creditsettings.php';
		$fp = fopen($cachefile, 'w');
		$s = "<?php\r\n";
