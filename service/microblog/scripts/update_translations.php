...
// Master MicroService .pot file location (created by update_pot.sh)
$microservice_pot = INSTALLDIR . '/locale/microservice.pot';
...
foreach ($languages as $language) {
    ...
    $file_url = 'http://translatewiki.net/w/i.php?' .
        http_build_query(array(
            ...
            'task' => 'export-to-file',
            'group' => 'out-microservice-core',
            'language' => $twnCode));

    $lcdir = INSTALLDIR . '/locale/' . $code;
    $msgdir = "$lcdir/LC_MESSAGES";
    $pofile = "$msgdir/microservice.po";
    $mofile = "$msgdir/microservice.mo";
    ...
    if (sha1($new_file) != $existingSHA1 || !file_exists($mofile)) {
        ...
        system(sprintf('msgmerge -U --backup=off %s %s', $pofile, $microservice_pot));
