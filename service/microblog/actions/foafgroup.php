...
class FoafGroupAction extends Action
{
    ...
    function handle($args)
    {
        ...
        while ($admins->fetch()) {
            ...
            $this->element('microservice:groupAdmin', array('rdf:resource' => $admin_uri));
        }
        ...
        foreach ($member_details as $uri => $details) {
            if ($details['is_admin'])
            {
                ...
                $this->elementStart('microservice:GroupAdminRole');
                $this->element('sioc:scope', array('rdf:resource' => $this->group->permalink()));
                $this->elementEnd('microservice:GroupAdminRole');
