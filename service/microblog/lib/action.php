...
class Action extends HTMLOutputter // lawsuit
{
    ...
    function showShortcutIcon()
    {
        if (is_readable(INSTALLDIR . '/theme/' . common_config('site', 'theme') . '/favicon.ico')) {
            ...
            $this->element('link', array('rel' => 'shortcut icon',
                                         'href' => common_path('favicon.ico', MicroService::isHTTPS())));
        }
    ...
    function showStylesheets()
    {
        if (Event::handle('StartShowStyles', array($this))) {
            ...
            // Use old name for MicroService for compatibility on events

            if (Event::handle('StartShowMicroServiceStyles', array($this)) &&
                Event::handle('StartShowLaconicaStyles', array($this))) {
                $this->primaryCssLink(null, 'screen, projection, tv, print');
                Event::handle('EndShowMicroServiceStyles', array($this));
                Event::handle('EndShowLaconicaStyles', array($this));
            }

            $this->cssLink(common_path('js/css/smoothness/jquery-ui.css', MicroService::isHTTPS()));

            if (Event::handle('StartShowUAStyles', array($this))) {
                $this->comment('[if IE]><link rel="stylesheet" type="text/css" '.
                               'href="'.Theme::path('css/ie.css', 'base').'?version='.MICROSERVICE_VERSION.'" /><![endif]');
                foreach (array(6,7) as $ver) {
                    if (file_exists(Theme::file('css/ie'.$ver.'.css', 'base'))) {
                        // Yes, IE people should be put in jail.
                        $this->comment('[if lte IE '.$ver.']><link rel="stylesheet" type="text/css" '.
                                       'href="'.Theme::path('css/ie'.$ver.'.css', 'base').'?version='.MICROSERVICE_VERSION.'" /><![endif]');
                    }
                }
                if (file_exists(Theme::file('css/ie.css'))) {
                    $this->comment('[if IE]><link rel="stylesheet" type="text/css" '.
                               'href="'.Theme::path('css/ie.css', null).'?version='.MICROSERVICE_VERSION.'" /><![endif]');
                }
        ...
        if (Event::handle('StartShowScripts', array($this))) {
            if (Event::handle('StartShowJQueryScripts', array($this))) {
                if (common_config('site', 'minify')) {
                    ...
                    $this->inlineScript('if (typeof window.JSON !== "object") { $.getScript("'.common_path('js/json2.min.js', MicroService::isHTTPS()).'"); }');
                    ...
                    $this->inlineScript('if (typeof window.JSON !== "object") { $.getScript("'.common_path('js/json2.js', MicroService::isHTTPS()).'"); }');
            ...
            if (Event::handle('StartShowMicroServiceScripts', array($this)) &&
                ...
                Event::handle('EndShowMicroServiceScripts', array($this));
    ...
    function showHeader()
    {
        ...
        $this->elementStart('div', array('class' => 'top-nav'));
        $this->showLogo();
        $this->showPrimaryNav();
    ...
    {
        $this->showMicroServiceLicense();
        $this->showContentLicense();
    }

    /**
     * Show MicroService license.
     *
     * @return nothing
     */
    function showMicroServiceLicense()
    {
    	/*
        if (common_config('site', 'broughtby')) {
            // TRANS: First sentence of the MicroService site license. Used if 'broughtby' is set.
            // TRANS: Text between [] is a link description, text between () is the link itself.
            // TRANS: Make sure there is no whitespace between "]" and "(".
            // TRANS: "%%site.broughtby%%" is the value of the variable site.broughtby
            $instr = _('**%%site.name%%** is a microblogging service brought to you by [%%site.broughtby%%](%%site.broughtbyurl%%).');
        } else {
            // TRANS: First sentence of the MicroService site license. Used if 'broughtby' is not set.
            $instr = _('**%%site.name%%** is a microblogging service.');
        }
        $instr .= ' ';
        // TRANS: Second sentence of the MicroService site license. Mentions the MicroService source code license.
        // TRANS: Make sure there is no whitespace between "]" and "(".
        // TRANS: Text between [] is a link description, text between () is the link itself.
        // TRANS: %s is the version of MicroService that is being used.
        $instr .= sprintf(_('It runs the [MicroService](http://www.microservice.com) microblogging software, version %s, available under the [GNU Affero General Public License](http://www.fsf.org/licensing/licenses/agpl-3.0.html).'), MICROSERVICE_VERSION);
        */
        
        $instr = "micro service mobile technology";
