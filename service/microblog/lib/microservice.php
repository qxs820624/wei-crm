...
class MicroService
{
    ...
    /**
     * Initialize, or re-initialize, MicroService global configuration
     * and plugins.
     *
    ...
    public static function init($server=null, $path=null, $conffile=null)
    {
        ...
        MicroService::initDefaults($server, $path);
        MicroService::loadConfigFile($conffile);

        $sprofile = common_config('site', 'profile');
        if (!empty($sprofile)) {
            MicroService::loadSiteProfile($sprofile);
        }
    ...
    {
        if ($nickname == MicroService::currentSite()) {
            return true;
        }
    ...
    protected function loadConfigFile($conffile=null)
    {
        ...
        if (isset($conffile)) {
            ...
            $config_files = array('/etc/microservice/microservice.php',
                                  '/etc/microservice/laconica.php',
                                  '/etc/laconica/laconica.php',
                                  '/etc/microservice/'.$_server.'.php',
                                  '/etc/laconica/'.$_server.'.php');

            if (strlen($_path) > 0) {
                $config_files[] = '/etc/microservice/'.$_server.'_'.$_path.'.php';
