...
function mail_confirm_address($user, $code, $nickname, $address, $url=null)
{
    ...
    // TRANS: %1$s is the addressed user's nickname, %2$s is the MicroService sitename,
...
function mail_subscribe_notify_profile($listenee, $other)
{
    if ($other->hasRight(Right::EMAILONSUBSCRIBE) &&
        ...
        // TRANS: %1$s is the subscribing user's nickname, %2$s is the MicroService sitename.
        $headers['Subject'] = sprintf(_('%1$s is now following you on %2$s.'),
                                      $other->getBestName(),
                                      common_config('site', 'name'));

        // TRANS: Main body of new-subscriber notification e-mail.
        // TRANS: %1$s is the subscriber's long name, %2$s is the MicroService sitename.
...
{
    // TRANS: Common footer block for MicroService notification emails.
    // TRANS: %1$s is the MicroService sitename,
...
function mail_new_incoming_notify($user)
{
    ...
    // TRANS: %s is the MicroService sitename.
    $headers['Subject'] = sprintf(_('New email address for posting to %s'),
                                  common_config('site', 'name'));

    // TRANS: Body of notification mail for new posting email address.
    // TRANS: %1$s is the MicroService sitename, %2$s is the e-mail address to send
...
function mail_notify_fave($other, $user, $notice)
{
    ...
    // TRANS: %5$s is a URL to all faves of the adding user, %6$s is the MicroService sitename,
...
function mail_notify_attn($user, $notice)
{
    ...
    $subject = sprintf(_('%1$s (@%2$s) sent a notice to your attention'), $bestname, $sender->nickname);
        ...
        // TRANS: %1$s is the sending user's name, $2$s is the MicroService sitename,
...
{
    $headers = array(
        'X-MicroService-MessageType' => $msg_type,
        'X-MicroService-TargetUser'  => $to,
        'X-MicroService-SourceUser'  => $from,
        'X-MicroService-Domain'      => common_config('site', 'server')
    );
...
function mail_notify_group_join($group, $joiner)
{
    ...
    while ($admin->fetch()) {
        ...
        if ($adminUser && $adminUser->email) {
            ...
            // TRANS: %1$s is the joining user's nickname, %2$s is the group name, and %3$s is the MicroService sitename.
            ...
            // TRANS: %1$s is the subscriber's long name, %2$s is the group name, and %3$s is the MicroService sitename,
