 ...
 * @link      http://www.microservice.com
 */
class DomainMicroServiceworkPlugin extends Plugin
{
    ...
    function initialize()
    {
        ...
        $nickname = MicroService::currentSite();
    ...
    function onAutoload($cls)
    {
        ...
        case 'GlobalrecoverAction':
            ...
            return false;
        case 'DomainMicroServiceworkInstaller':
    ...
    function onRouterInitialized($m)
    {
        if (common_config('globalapi', 'enabled')) {
            foreach (array('register', 'login', 'recover') as $method) {
                $m->connect('api/microservice/global/'.$method,
    ...
    {
        $versions[] = array('name' => 'DomainMicroServicework',
                            'version' => MICROSERVICE_VERSION,
                            'author' => 'Evan Prodromou',
                            'homepage' => 'http://www.microservice.comwiki/Plugin:DomainMicroServicework',
                            'rawdescription' =>
    ...
    static function userExists($email)
    {
        ...
        MicroService::switchSite($sn->nickname);
    ...
    static function registerEmail($email)
    {
        ...
        if (empty($sn)) {
            $installer = new DomainMicroServiceworkInstaller($domain);

            // Do the thing
            $installer->main();

            $sn = $installer->getMicroServicework();
...
DomainMicroServiceworkPlugin::$_thetree = $tldTree;
