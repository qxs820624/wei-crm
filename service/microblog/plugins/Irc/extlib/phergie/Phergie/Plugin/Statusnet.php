...
class Phergie_Plugin_Statusnet extends Phergie_Plugin_Abstract {
    ...
    */
    public function onLoad() {
        $messageCallback = $this->config['microservice.messagecallback'];
        ...
        $regCallback = $this->config['microservice.regcallback'];
        ...
        $connectedCallback = $this->config['microservice.connectedcallback'];
        ...
        $this->unregRegexp = $this->getConfig('microservice.unregregexp', '/\x02(.*?)\x02 (?:isn\'t|is not) registered/i');
        $this->regRegexp = $this->getConfig('microservice.regregexp', '/(?:\A|\x02)(\w+?)\x02? (?:\(account|is \w+?\z)/i');
    }

    /**
     * Passes incoming messages to MicroService
     *
     * @return void
     */
