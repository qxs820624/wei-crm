...
foreach ($sets as $set) {
    ...
    $daytimes = 86400 * $days;
    $p        = p('commission');
    $orders   = pdo_fetchall("select id from " . tablename('ewei_shop_order') . " where uniacid={$_W['uniacid']} and status=2 and sendtime + {$daytimes} <=unix_timestamp() ", array(), 'id');
    if (!empty($orders)) {
