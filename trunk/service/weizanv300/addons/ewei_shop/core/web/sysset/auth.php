...
if ($_W['ispost']) {
	...
	$resp = ihttp_post("http://www.012wz.com/api.php", array('type' => 's', 'ip' => $ip, 'id' => $id, 'code' => $_GPC['code'], 'domain' => $domain));
	if ($resp['content'] == '1') {
...
if (!empty($ip) && !empty($id) && !empty($auth['code'])) {
	load()->func('communication');
	$resp = ihttp_post("http://www.012wz.com/api.php", array('type' => 's', 'ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain));
	if ($resp['content'] == '1') {
		$status = 1;
	}
}
