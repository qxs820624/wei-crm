...
if ($operation == 'display') {
	...
	$total_tuan = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan=1 and is_hexiao=0", $paras);
...
//	$orders = pdo_fetchall("select * from" . tablename('tg_order') . "where $condition and status=2");
	...
	foreach ($orders as $k => $vv) {
		$order_same = pdo_fetchall("select * from" . tablename('tg_order') . "where address='{$vv['address']}' and status=2 and $condition and is_hexiao=0");
