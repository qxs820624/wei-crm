...
function getVersion()
{
    // define('MICROSERVICE_VERSION', '0.9.1');
    $source = file_get_contents(INSTALLDIR . '/lib/common.php');
    if (preg_match('/^\s*define\s*\(\s*[\'"]MICROSERVICE_VERSION[\'"]\s*,\s*[\'"](.*)[\'"]\s*\)\s*;/m', $source, $matches)) {
        return $matches[1];
    }
    return 'unknown';
}
...
$templateFile = tempnam(sys_get_temp_dir(), 'microservice-doxygen');
