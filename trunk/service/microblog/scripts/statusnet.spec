...
Source:         microservice-%{version}.tar.gz
Group:          Applications/Internet
Summary:        MicroService, the Open Source microblogging platform
BuildArch:      noarch
...
%description
From the ABOUT file: MicroService (pronounced "luh-KAWN-ih-kuh") is a Free
...
mkdir -p %{buildroot}%{_datadir}/microservice
cp -a db %{buildroot}%{_datadir}/microservice/db

mkdir -p %{buildroot}%{_datadir}/microservice/avatar

mkdir -p %{buildroot}%{_sysconfdir}/httpd/conf.d
cat > %{buildroot}%{_sysconfdir}/httpd/conf.d/microservice.conf <<"EOF"
Alias /microservice/ "/var/www/microservice/"

<Directory "/var/www/microservice">
...
%dir %{wwwpath}
%{wwwpath}/*
%{_datadir}/microservice/*
%attr(-,apache,apache) %dir %{_datadir}/microservice/avatar
%doc COPYING README doc-src/*
%config(noreplace) %{_sysconfdir}/httpd/conf.d/microservice.conf
...
- Factored microservice version to the first line of the file.
...
- packaged microservice version 0.7.1

* Wed Feb 04 2009 tuukka.pasanen@ilmi.fi
- packaged microservice version 0.7.0 using the buildservice spec file wizard
