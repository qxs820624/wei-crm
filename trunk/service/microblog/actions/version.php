...
class VersionAction extends Action
{
    ...
    function title()
    {
        // TRANS: Title for version page. %s is the MicroService version.
        return sprintf(_("MicroService %s"), MICROSERVICE_VERSION);
    }
    ...
    */
    ...
    function showContent()
    {
        ...
        // TRANS: Content part of MicroService version page.
        // TRANS: %1$s is the engine name (MicroService) and %2$s is the MicroService version.
        $this->raw(sprintf(_('This site is powered by %1$s version %2$s, '.
                             'Copyright 2008-2011 MicroService, Inc. '.
                             'and contributors.'),
                           XMLStringer::estring('a', array('href' => 'http://www.microservice.com'),
                                                // TRANS: Engine name.
                                                _('MicroService')),
                           MICROSERVICE_VERSION));
        $this->elementEnd('p');

        // TRANS: Header for MicroService contributors section on the version page.
        $this->element('h2', null, _('Contributors'));

        sort($this->contributors);
        $this->element('p', null, implode(', ', $this->contributors));

        // TRANS: Header for MicroService license section on the version page.
        $this->element('h2', null, _('License'));

        $this->element('p', null,
                       // TRANS: Content part of MicroService version page.
                       _('MicroService is free software: you can redistribute it and/or modify '.
                         'it under the terms of the GNU Affero General Public License as published by '.
                         'the Free Software Foundation, either version 3 of the License, or '.
                         '(at your option) any later version.'));
        ...
        if (count($this->pluginVersions)) {
            // TRANS: Header for MicroService plugins section on the version page.
    ...
    var $contributors = array('Evan Prodromou (MicroService)',
                              'Zach Copley (MicroService)',
                              'Earle Martin (MicroService)',
                              'Marie-Claude Doyon (MicroService)',
                              'Sarven Capadisli (MicroService)',
                              'Robin Millette (MicroService)',
                              'Ciaran Gultnieks',
                              'Michael Landers',
                              'Ori Avtalion',
                              'Garret Buell',
                              ...
                              'Brett Taylor',
                              'Brigitte Schuster',
                              'Brion Vibber (MicroService)',
                              'Siebrand Mazeland',
                              'Samantha Doherty (MicroService)');
}
