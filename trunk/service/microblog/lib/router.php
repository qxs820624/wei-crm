    ...
    * There can still be problems with a) differences in versions of the plugins and
    ...
    function initialize()
    {
        ...
        if (Event::handle('StartInitializeRouter', array(&$m))) {
            ...
            $m->connect('api/microservice/version.:format',
                        array('action' => 'ApiMicroserviceVersion',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/config.:format',
                        array('action' => 'ApiMicroserviceConfig',
                              'format' => '(xml|json)'));
            ...
            $m->connect('api/microservice/groups/timeline/:id.:format',
                        array('action' => 'ApiTimelineGroup',
                              'id' => Nickname::INPUT_FMT,
                              'format' => '(xml|json|rss|atom|as)'));

            $m->connect('api/microservice/groups/show/:id.:format',
                        array('action' => 'ApiGroupShow',
                              'id' => Nickname::INPUT_FMT,
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/show.:format',
                        array('action' => 'ApiGroupShow',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/join/:id.:format',
                        array('action' => 'ApiGroupJoin',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/join.:format',
                        array('action' => 'ApiGroupJoin',
                              'id' => Nickname::INPUT_FMT,
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/leave/:id.:format',
                        array('action' => 'ApiGroupLeave',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/leave.:format',
                        array('action' => 'ApiGroupLeave',
                              'id' => Nickname::INPUT_FMT,
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/is_member.:format',
                        array('action' => 'ApiGroupIsMember',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/list/:id.:format',
                        array('action' => 'ApiGroupList',
                              'id' => Nickname::INPUT_FMT,
                              'format' => '(xml|json|rss|atom)'));

            $m->connect('api/microservice/groups/list.:format',
                        array('action' => 'ApiGroupList',
                              'format' => '(xml|json|rss|atom)'));

            $m->connect('api/microservice/groups/list_all.:format',
                        array('action' => 'ApiGroupListAll',
                              'format' => '(xml|json|rss|atom)'));

            $m->connect('api/microservice/groups/membership/:id.:format',
                        array('action' => 'ApiGroupMembership',
                              'id' => Nickname::INPUT_FMT,
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/membership.:format',
                        array('action' => 'ApiGroupMembership',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/create.:format',
                        array('action' => 'ApiGroupCreate',
                              'format' => '(xml|json)'));

            $m->connect('api/microservice/groups/update/:id.:format',
                        array('action' => 'ApiGroupProfileUpdate',
                              'id' => '[a-zA-Z0-9]+',
                              'format' => '(xml|json)'));
                              
            $m->connect('api/microservice/conversation/:id.:format',
            ...
            // Tags
            $m->connect('api/microservice/tags/timeline/:tag.:format',
                        array('action' => 'ApiTimelineTag',
                              'format' => '(xml|json|rss|atom|as)'));

            // media related
            $m->connect(
                'api/microservice/media/upload',
            ...
            $m->connect('api/trends/place.json', array('action' => 'ApiTrendsPlace'));
            
            $m->connect('api/geo/place.json', array('action' => 'ApiGeoPlace'));
            $m->connect('api/geo/reverse_geocode.json', array('action' => 'ApiGeoReverseGeocode'));
            $m->connect('api/geo/search.json', array('action' => 'ApiGeoSearch'));
            $m->connect('api/geo/similar_places.json', array('action' => 'ApiGeoSimilarPlaces'));
            ...
            $m->connect('api/microservice/app/service/:id.xml',
                        array('action' => 'ApiAtomService'),
                        array('id' => Nickname::DISPLAY_FMT));

            $m->connect('api/microservice/app/service.xml',
                        array('action' => 'ApiAtomService'));

            $m->connect('api/microservice/app/subscriptions/:subscriber/:subscribed.atom',
                        array('action' => 'AtomPubShowSubscription'),
                        array('subscriber' => '[0-9]+',
                              'subscribed' => '[0-9]+'));

            $m->connect('api/microservice/app/subscriptions/:subscriber.atom',
                        array('action' => 'AtomPubSubscriptionFeed'),
                        array('subscriber' => '[0-9]+'));

            $m->connect('api/microservice/app/favorites/:profile/:notice.atom',
                        array('action' => 'AtomPubShowFavorite'),
                        array('profile' => '[0-9]+',
                              'notice' => '[0-9]+'));

            $m->connect('api/microservice/app/favorites/:profile.atom',
                        array('action' => 'AtomPubFavoriteFeed'),
                        array('profile' => '[0-9]+'));

            $m->connect('api/microservice/app/memberships/:profile/:group.atom',
                        array('action' => 'AtomPubShowMembership'),
                        array('profile' => '[0-9]+',
                              'group' => '[0-9]+'));

            $m->connect('api/microservice/app/memberships/:profile.atom',
