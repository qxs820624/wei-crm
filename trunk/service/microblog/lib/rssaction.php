...
class Rss10Action extends Action
{
    ...
    function prepare($args)
    {
        ...
        if (common_config('site', 'private')) {
            if (!isset($_SERVER['PHP_AUTH_USER'])) {
                ...
                header('WWW-Authenticate: Basic realm="MicroService RSS"');
    ...
    function showItem($notice)
    {
        ...
        if ($location && isset($location->lat) && isset($location->lon)) {
            ...
            $this->element('microservice:origin', $attrs);
        }
        $this->element('microservice:postIcon', array('rdf:resource' => $profile->avatarUrl()));
        $this->element('cc:licence', array('rdf:resource' => common_config('license', 'url')));
        if ($notice->reply_to) {
