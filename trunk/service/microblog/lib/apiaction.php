...
/*! @mainpage MicroService REST API
    ...
    relative to the MicroService API root. The API root is determined by the
...
class ApiAction extends Action
{
    ...
    {
        MicroService::setApi(true); // reduce exception reports to aid in debugging
        parent::prepare($args);
        ...
        if ($this->arg('since')) {
            header('X-MicroService-Warning: since parameter is disabled; use since_id');
        }
    ...
    function twitterUserArray($profile, $get_notice=false)
    {
        ...
        $twitter_user['microservice_blocking'] = false;
        $twitter_user['notifications'] = false;

        if (isset($this->auth_user)) {

            $twitter_user['following'] = $this->auth_user->isSubscribed($profile);
            $twitter_user['microservice_blocking']  = $this->auth_user->hasBlocked($profile);
        ...
        $twitter_user['microservice_profile_url'] = $profile->profileurl;
    ...
    function twitterSimpleStatusArray($notice, $include_user=true)
    {
        ...
        $twitter_status['truncated'] = false; # Not possible on MicroService
        ...
        $twitter_status['microservice_html'] = $notice->rendered;
        $twitter_status['microservice_conversation_id'] = intval($notice->conversation);
    ...
    function showTwitterXmlStatus($twitter_status, $tag='status', $namespaces=false)
    {
        $attrs = array();
        if ($namespaces) {
            $attrs['xmlns:microservice'] = 'http://www.microservice.comschema/api/1/';
        }
        ...
        foreach($twitter_status as $element => $value) {
            ...
            case 'retweeted_status':
                ...
                break;
            default:
                if (strncmp($element, 'microservice_', 10) == 0) {
                    $this->element('microservice:'.substr($element, 10), null, $value);
                } else {
                    $this->element($element, null, $value);
                }
            }
    ...
    function showTwitterXmlUser($twitter_user, $role='user', $namespaces=false)
    {
        ...
        foreach($twitter_user as $element => $value) {
            ...
            } else if (strncmp($element, 'microservice_', 10) == 0) {
                $this->element('microservice:'.substr($element, 10), null, $value);
            } else {
                $this->element($element, null, $value);
            }
    ...
    function showXmlTimeline($notice)
    {
        ...
        $this->elementStart('statuses', array('type' => 'array',
                                              'xmlns:microservice' => 'http://www.microservice.comschema/api/1/'));
