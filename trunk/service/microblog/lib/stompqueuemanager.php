...
class StompQueueManager extends QueueManager
{
    ...
    function incDeliveryCount($msgId)
    {
	    ...
	    if ($cache) {
		    $key = 'microservice:stomp:message-retries:' . $msgId;
    ...
    {
        if ($site != MicroService::currentSite()) {
            $this->stats('switch');
            MicroService::switchSite($site);
            $this->initialize();
        }
    }
     ...
     * triggered by a broadcast to the 'microservice-control' topic.
     *
     ...
     *
     * /queue/microservice/main
     * /queue/microservice/main/distrib
     * /queue/microservice/xmpp/xmppout/site01
     *
     * @param string $queue
     * @return string
     */
