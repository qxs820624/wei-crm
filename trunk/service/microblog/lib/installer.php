 ...
 * Copyright (C) 2009-2010, MicroService, Inc.
 *
 ...
 * @author   gaoyuan tan <gaoyuantan@163.com>
 * @author   gaoyuan tan <gaoyuantan@163.com>
 * @copyright 2009-2010 MicroService, Inc http://status.net
...
abstract class Installer
{
    ...
    function checkPrereqs()
    {
        ...
        if ($str != $replaced) {
            $this->warning('PHP is linked to a version of the PCRE library ' .
                           ...
                           'http://www.microservice.comwiki/Red_Hat_Enterprise_Linux#PCRE_library' .
    ...
    function validateAdmin()
    {
        ...
        if (empty($this->adminNick)) {
            $this->updateStatus("No initial MicroService user nickname specified.", true);
            $fail = true;
        }
        ...
        if (empty($this->adminPass)) {
            $this->updateStatus("No initial MicroService user password specified.", true);
            $fail = true;
        }
     ...
     * Side effect: may load portions of MicroService framework.
    ...
    function doInstall()
    {
        ...
        if (!defined('MICROSERVICE')) {
            define('MICROSERVICE', 1);
        }
        require_once INSTALLDIR . '/lib/framework.php';
        MicroService::initDefaults($this->server, $this->path);
        ...
        if (!empty($this->adminNick)) {
            ...
            if ($this->registerInitialUser()) {
                ...
                $this->updateStatus(
                    "Could not create initial MicroService user (administrator).",
                    true
                );
                return false;
            }
        ...
        */
        ...
        $this->updateStatus("MicroService has been installed at $link");
        $this->updateStatus(
            "<strong>DONE!</strong> You can visit your <a href='$link'>new MicroService site</a> (login as '$this->adminNick'). If this is your first MicroService install, you may want to poke around our <a href='http://www.microservice.comwiki/Getting_started'>Getting Started guide</a>."
        );
