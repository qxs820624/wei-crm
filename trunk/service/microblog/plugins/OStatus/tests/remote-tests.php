...
class SNTestClient extends TestBase
{
    ...
    function createGroup($nickname, $options=array()) {
        ...
        $data = $this->api('microservice/groups/create', 'json',
    ...
    function groupInfo($nickname) {
        $data = $this->api('microservice/groups/show', 'json', array(
            'id' => $nickname
        ));
    }
...
remote-tests.php [options] <url1> <url2>
  ...
  url1: base URL of a MicroService instance
  url2: base URL of another MicroService instance

This will register user accounts on the two given MicroService instances
