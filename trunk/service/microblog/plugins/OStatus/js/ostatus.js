...
SN.Init.OStatusCookie = function() {
    if (SN.U.MicroServiceInstance.Get() === null) {
        SN.U.MicroServiceInstance.Set({RemoteProfile: null});
    }
};
                ...
                success: function(data, textStatus, xhr) {
                    if (typeof($('form', data)[0]) != 'undefined') {
                        ...
                        if (form.attr('id') == 'form_ostatus_connect') {
                            ...
                            form.find('#profile').val(SN.U.MicroServiceInstance.Get().RemoteProfile);

                            form.find("[type=submit]").bind('click', function() {
                                SN.U.MicroServiceInstance.Set({RemoteProfile: form.find('#profile').val()});
                                return true;
                            });
                        }
                    }
