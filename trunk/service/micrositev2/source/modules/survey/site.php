...
class SurveyModuleSite extends WeModuleSite {
	...
	public function doWebQuery() {
		global $_W, $_GPC;
		$kwd = $_GPC['keyword'];
		$sql = 'SELECT * FROM ' . tablename('survey') . ' WHERE `weid`=:weid AND `title` LIKE :title AND suggest_status = 1 AND endtime > :endtime ORDER BY sid DESC LIMIT 0,8';
		$params = array();
		$params[':weid'] = $_W['weid'];
		$params[':title'] = "%{$kwd}%";
		$params[':endtime'] = time();
	...
	public function doWebManage() {
		...
		$sql = 'SELECT * FROM ' . tablename('survey_fields') . ' WHERE `sid`=:sid AND `type` !=:type ORDER BY displayorder ASC,sfid ASC';
		$params = array();
		$params[':sid'] = $sid;
			...
			foreach($list as &$r) {
				...
				foreach($fdatas as $fd) {
					if($sta[$fd['sfid']]['type'] == 'checkbox') {
						$a[$fd['srid']][] = $fd['data']; 
						$r['fields'][$fd['sfid']] = implode(' ',$a[$fd['srid']]);
					} else {
